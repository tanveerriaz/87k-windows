...
describe("extractCapsule error handling", () => {
  ...
  it("maps a non-JSON 503 body to a friendly message", async () => {
    ...
    await expect(extractCapsule({ roomCode: "demo87", memory: "m".repeat(20) })).rejects.toThrow(/couldn't reach|try again/i);
  });
  ...
  it("surfaces LOCAL_GEMMA_BUSY as a retryable message", async () => {
    ...
    await expect(extractCapsule({ roomCode: "demo87", memory: "m".repeat(20) })).rejects.toMatchObject({ code: "LOCAL_GEMMA_BUSY" });
  });
});
